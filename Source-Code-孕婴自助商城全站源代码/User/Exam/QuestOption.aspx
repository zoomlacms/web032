<%@ page language="C#" autoeventwireup="true" validaterequest="false" masterpagefile="~/Common/Common.master" inherits="User_Exam_QuestOption, App_Web_pwji0luf" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<style>
    *{font-size:14px;}
    .question_option .quescon {padding:5px;border:1px solid #ddd;height:150px;margin-top:5px;}
    ...
    #answer_ue_div {width:500px;display:none;position:fixed;top:30%;right:30%; border:1px solid #ddd;box-shadow:0 4px 20px 1px rgba(0,0,0,0.2);background:#ffffff;}
                ...
                <div class="margin_t10 panel panel-info question_body" data-id="{{item.id}}">
                    <div class="panel-heading">
                        <div class="opts_title">
                            <span>填空<input type="text" class="form-control text_xs text-center" value="{{item.id+1}}" /> 选项:</span>
...
<div class="footfix_div text-center">
     <button type="button" class="btn btn-primary" onclick="CheckData()">保存</button>
        <a href="javascript:;" onclick="parent.CloseComDiag()" class="btn btn-primary">关闭</a>
</div>
        ...
        function InitData() {
            ...
            for (var i = 0; i < conarr.length - 1; i++) {
                if (page.scope.list[i]) {//如果有数据
                    ...
                    questdata.push({ id: i,num:curlist.num, title: curlist.title, score: curlist.score, opts: curlist.opts });
                    continue;
                }
                var childopts =[];
                ...
                questdata.push({ id: i,num:4, title:"", score: 5, opts: childopts});
            }
            UpdateView();
        }
